...
| Field | Type | Set When |
|---|---|---|
| `name` | string PK | On insert, per the doctype's naming rule (`Naming and Autoname Rules.md`). |
...
| `docstatus` | 0/1/2 | See `Submittable Document Lifecycle.md`. |
...
list (`Expense Claim`, `Payroll Entry`, `Salary Slip`, `Leave Encashment`, `Gratuity`)
additionally marks these as needing a stricter/longer-retained audit trail — treat
this list as the doctypes where a port absolutely needs field-level change history
(who changed what value to what, when), even if you don't build full versioning for
every table.
...
nuanced (see `Company`'s `handle_linked_docs` in `Cross-Doctype Hooks (doc_events).md`).
