...
This is a child (table) doctype, used exclusively as the row type for `Interview Type.interviewers` (a Table MultiSelect field — see `Interview Type.md`).
...
- `hrms.hr.doctype.interview.interview.get_interviewers(interview_type)` — queries `frappe.get_all("Interviewer", filters={"parent": interview_type}, fields=["user as interviewer"])` to build the list of interviewers for a given Interview Type when scheduling/creating an Interview (see `Interview.md`).
...
`"permissions": []` in the JSON — child tables inherit access from their parent document (Interview Type); there are no independent permission rules to port for this table. Refer to `Interview Type.md`'s Permissions section for the effective access rules.
