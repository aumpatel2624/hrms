...
| checkin_radius | Checkin Radius | Int | — | no | — | no | non-negative; "Radius within which check-in is allowed (in meters)"; `0` or unset disables the radius check (see `Employee Checkin.md` validation step 6.e) |
...
None. Read (not written) by `Employee Checkin.validate_distance_from_shift_location` (see `Employee Checkin.md`) at checkin-time, via the employee's active `Shift Assignment.shift_location`.
